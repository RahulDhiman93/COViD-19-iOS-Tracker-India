...
class ErrorView: UIView {
    ...
    override func awakeFromNib() {
       ...
       // self.statusIcon1.image = UIImage(named: "Close")!.withRenderingMode(.alwaysTemplate)
        ...
        keyboardChange = KeyBoard { [weak self] newHeight in
            self?.frame.origin.y = UIScreen.main.bounds.height - newHeight - (self?.frame.height ?? (self?.kHeight)!)
        }
    }
    ...
    func setErrorMessage(message:String, isError:Bool) {
        ...
        let size = message.heightWithConstrainedWidth(width: UIScreen.main.bounds.width - 57, font: UIFont.poppinsFontRegular(size: 15)!)
    ...
    static func showWith(message: String, isErrorMessage: Bool = true, removed: (() -> Void)?) {
        ...
        errorMessageView.frame = CGRect(x: 0, y: UIScreen.main.bounds.height, width: UIScreen.main.bounds.width, height: 45)
